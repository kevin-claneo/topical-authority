...
def init_session_state():
    """
    Initializes or updates the Streamlit session state variables for property selection,
...
def fetch_gsc_data(webproperty, search_type, start_date, end_date, dimensions, max_position, min_clicks, device_type=None):
    ...
    try:
        ...
        if 'clicks' in df.columns and 'position' in df.columns:
            df = df[(df['position'] <= 5) & (df['clicks'] >= min_clicks)]  # Filter queries based on position and clicks
        else:
...
def show_fetch_data_button(webproperty, search_type, start_date, end_date, selected_dimensions, max_position, min_clicks):
    ...
    if st.button("Fetch Data"):
        ...
        if report is not None:
            ...
            download_csv_link(main_queries_df)
...
def main():
    ...
    Handles the app setup, authentication, UI components, data fetching, entity generation, relationship generation, and semantic map visualization.
    """
    setup_streamlit()
    ...
    if auth_code and not st.session_state.get('credentials'):
        try:
            ...
            st.warning(f"Error details: {e.__dict__}")
